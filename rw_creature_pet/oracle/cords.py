...
固定 40 Hz 的位置约束绳索；只影响外观，允许局部自然探出边缘活动带。
...
class Rope:
    ...
    def step(self, pins, *, end_pull=None, forces=None, supply=None, inputs=()):
        key = (tuple(pins.items()), end_pull, inputs, supply)
        ...
        if end_pull is not None:
            # 原版末端超出 80 后按超出量拉回；每 tick 施加一次，
            # 不在每轮约束中投影到圆周，让绳长和惯性仍能决定位置。
            center, radius = end_pull
            dx, dy = x[-1]-center.x, y[-1]-center.y
            distance = hypot(dx, dy)
            if distance > radius:
                pull = (distance-radius)*.25/distance
                x[-1] -= dx*pull
                y[-1] -= dy*pull
...
class OracleCords:
    ...
    FINE_COUNT = 14
    FINE_POINTS = 20
    JUNCTION_PULL_RADIUS = 80.
    MAIN_TAIL_LENGTH = 190.  # 原版导向点 60 到末端 79：19 段，每段 10。
    ...
    def __init__(self, scene, head):
        ...
        for index in range(self.FINE_COUNT):
            short = rng.random() < .5
            value = rng.random()
            length = 50.+15.*value if short else 50.+150.*value**1.5
            # 原版 20 个节点、19 段，每段取长度参数 / 20。
            length *= (self.FINE_POINTS-1)/self.FINE_POINTS
            direction = Vec2(rng.uniform(-1., 1.), rng.uniform(-.6, .6))
            self.fine_lengths.append(length)
    ...
    @classmethod
    def supply_lengths(cls, first, last):
        a, b = path_length(first), path_length(last)
        return (max(65., a+min(36., a*.18+12)),
                max(cls.MAIN_TAIL_LENGTH, b+min(32., b*.20+12)))
    ...
    def step(self, scene, head, upper, direction, look):
        ...
        # 分线端可垂到身体下方；柔性拉回限制远离，不钉在固定圆周上。
        self.main.step({0: scene.base, 60: self.guide},
                       end_pull=(upper, self.JUNCTION_PULL_RADIUS), forces=force,
        ...
        for index, rope in enumerate(self.fine):
            ...
            # 分叉点快速远离时先补足必需跨度，不能让缓慢供线把末段拉长。
            # 额外松弛和回收仍走 Rope 的平滑供线，长短个体长度保持不变。
            minimum = span/(self.FINE_POINTS-1)
            if rope.rest[0] < minimum:
                rope.rest[:] = [minimum]*(self.FINE_POINTS-1)
            # 绳长和两端连接自然限制覆盖范围，避免把长线挤进半径 32 的圆内。
            rope.step({0: junction, 19: head}, forces=forces,
